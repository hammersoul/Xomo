...
final class ParseCurrencies {
    ...
    public  var currencies = [CurrenciesModel]()

    func parse(completion: @escaping (([CurrenciesModel]) -> Void), page: String = "1") {
        DispatchQueue.global().asyncAfter(deadline: .now()) { [weak self] in
            if let url = URL(string: self!.url + page) {
                do {
                    ...
                    for index in 0..<elementsTicker.count {
                        let currency = try CurrenciesModel(name: elementsName[index].text(), ticker: elementsTicker[index].text(), priceOne: elementsPriceOne[index].text(), priceTwo: elementsPriceTwo[index].text())
